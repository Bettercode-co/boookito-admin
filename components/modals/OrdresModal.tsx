...
const OrdresModal = ({ setIsModalOpen, isModalOpen }) => {
  ...
  const fetchNationCode = () => {
    axiosInstance.get(`admin/user/search/${nationId}`)
    .then(response =>  setUserData(response.data))
      nationCodeOptionFilter(userData)
    // .then(res => console.log(nationOption))
  }
  const fetchBook = () => {
    axiosInstance.get(`admin/book/search/${bookId}`)
    .then(response =>  setBookData(response.data))
     bookOptionFilter(bookData)
  }
  ...
  const nationCodeOptionFilter = (data) => {
    const dataArray = data.map(item => {
      return({
       value: item.username,
       label: `${PN.convertEnToPe(item.username)} - ${item.firstname} ${item.lastname}`,
     }
 )  })
 ...
 setBookOption(dataArray);
  ...
  return (
    <>
      {isModalOpen ? (
    <div className={`w-full h-full  flex justify-center items-center fixed t-0 r-0  bg-gray-300 bg-opacity-50 transition-all duration-300 ease-in`
          ...
          onClick={eventHandler}
           ...
           >
            ...
            <label htmlFor="nationCode">
                کد ملی
             <Select id="nationCode" className=" w-64" onInputChange={(e) => setNationId(e)} options={nationOption} placeholder="کد ملی متقاضی" />
            </label>
            <label htmlFor="bookCode">
              کد کتاب 
             <Select  id="bookCode" className=" w-64" onInputChange={(e) =>  setBookId(e)}  options={bookOption} placeholder="کد کتاب" />
            </label>
            <label htmlFor="duration">
              مدت سفارش
             <Select id="duration" isSearchable={false} className=" w-64" defaultValue={durationOption[0]} options={durationOption}  placeholder="مدت سفارش" />
            </label>
